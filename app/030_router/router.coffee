...
if Meteor.isServer
  try
    ...
    # Check if CGU exists
    if Pages.find(slug: 'cgu').count() is 0
    ...
    if Pages.find(slug: 'cookie').count() is 0
...
Router.route '/cgu', ->
  @render 'nav', to: 'nav', data: ->
    ...
    , (page) -> page.order
  @render 'basicPage', data: -> Pages.findOne({slug: 'cgu'})
  @render 'footer', to: 'footer', data: ->
...
Router.route '/cookie', ->
  @render 'nav', to: 'nav', data: ->
    _.sortBy (_.filter pages, (page) -> page.display is 1 or page.display is 2)
    , (page) -> page.order
  @render 'basicPage', data: -> Pages.findOne({slug: 'cookie'})
  @render 'footer', to: 'footer', data: ->
    _.sortBy (_.filter pages, (page) -> page.display is 1 or page.display is 3)
    , (page) -> page.order
, fastRender: true
